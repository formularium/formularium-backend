...
from forms.services import FormService, FormServiceException, FormReceiverService, EncryptionKeyService, \
    FormSchemaService, FormTranslationService
...
class FormSchemaServiceTest(TestCase):
    ...
    def test_add_translation(self):
        translation = FormTranslationService.create_form_translation(self.user,
                                                                     form_id=self.form.id, language="de", region="DE")
        self.assertEqual(translation.language, "de")

        with self.assertRaises(FormServiceException):
            translation = FormTranslationService.create_form_translation(self.user,
                                                                         form_id=self.form.id, language="NOTALanguage",
                                                                         region="DE")
    ...
    def test_update_translation(self):
        ...
        translation = FormTranslationService.update_form_translation(self.user, translation_id=translation.id,
                                                                     language="en", region="US", active=True)
        self.assertEqual(translation.language, "en")
        self.assertEqual(translation.active, True)

    def test_update_translation_key(self):
        translation = FormTranslationService.create_form_translation(self.user,
                                                                     form_id=self.form.id, language="en", region="US")
        translated_key = FormTranslationService.update_translation_string(user=self.user,
                                                                          translation_id=translation.pk, key="a.greeting",
                                                                          value="Hey there!")
