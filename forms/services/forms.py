...
from teams.permissions import CanActivateEncryptionKeyPermission
...
class FormService(Service, CRUDMixin):
    ...
    @classmethod
    def retrieve_public_keys_for_form(cls, form_id: int) -> [Team]:
        """
        ...
        return Team.objects.filter(forms__in=[form]).all()
    ...
    @classmethod
    def update_form_teams(
        cls, user: AbstractUser, form_id: int, teams: List[int]
    ) -> Form:
        """
        update the user-teams that receive the form submissions
        :param user: the user calling the service
        :param form_id: the id of the form affected
        :param teams: a list of teams that should be able to receive submissions
        :return: the updated form instance
        """
        if not user.has_perm(CanActivateEncryptionKeyPermission):
            raise PermissionError("You are not allowed to create this form.")
        ...
        for grp in teams:
            ...
            form.teams.add(Team.objects.get(pk=grp))
...
class FormReceiverService(Service):
    ...
    def retrieve_accessible_forms(cls, user: AbstractUser) -> [Form]:
        ...
        """
        return Form.objects.filter(teams__in=[t.team for t in user.memberships.all()])
