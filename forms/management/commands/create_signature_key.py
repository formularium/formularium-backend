...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for key_id, registered_subkey in key.subkeys.items():
            current_key = key
            current_key.protect(
                settings.SECRET_KEY, SymmetricKeyAlgorithm.AES256, HashAlgorithm.SHA256
            )
            SignatureKey.objects.create(
                public_key=str(current_key.pubkey),
                private_key=str(current_key),
                subkey_id=key_id,
                key_type=SignatureKey.SignatureKeyType.SECONDARY,
                active=True,
            )
